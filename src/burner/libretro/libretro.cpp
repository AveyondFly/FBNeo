...
void retro_set_environment(retro_environment_t cb)
{
	...
	static const struct retro_subsystem_rom_info subsystem_rom[] = {
		{ "Rom", "zip|7z|fid|lww", true, true, true, NULL, 0 },
	};
...
#define GIT_VERSION ""
	...
	info->valid_extensions = "zip|7z|fid|lww|cue|ccd";
    ...
    if ((fp = fopen(patch_name, "rb")) != NULL) {
        ...
        while (!feof(fp)) {
			if (fgets(s, sizeof(s), fp) != NULL) {
				...
				_stprintf(ips_path, _T("%s%c%s%c%s%c%s%s"),
						g_rom_dir,
...
static void prepare_ips_data(const char *filename)
{
	UINT32 nActivePatches = 0;
	FILE* fp = NULL;
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (len > 4 && strstr(&line[len - 4], ".dat") != NULL) {
			_stprintf(szIpsActivePatches[nActivePatches++], _T("%s%c%s%c%s%c%s"),
					g_rom_dir,
					PATH_DEFAULT_SLASH_C(),
					"ips",
					PATH_DEFAULT_SLASH_C(),
...
{
	size_t len_str;
#define IPS_ROM_SUFFIX_LEN 4
	size_t len_suffix = IPS_ROM_SUFFIX_LEN;

	if (!info)
		return false;

	len_str = strlen(info->path);
	if (len_str >= len_suffix &&
	   (strstr(info->path + len_str - IPS_ROM_SUFFIX_LEN, ".lww") ||
	    strstr(info->path + len_str - IPS_ROM_SUFFIX_LEN, ".fid")))
		return retro_get_real_rom_name(info->path);
